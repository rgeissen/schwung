...
const m = src.match(/let padBlockWanted = false;\nfunction reconcilePadBlock\(\) \{[\s\S]*?\n\}/);
...
const fn = new Function("VIEWS", "host_pad_block", "coRunUiActive",
    ...
    (VIEWS, host_pad_block, coRunUiActive, isTextEntryActive,
     () => view, () => coRunView, shadow_get_display_mode,
     shadow_get_pad_led_snapshot, move_midi_internal_send);
...
/* And it does not thrash: nothing more to release on later ticks. */
step("still released, no repeat writes",
     () => { blocked = "untouched"; }, "untouched");
